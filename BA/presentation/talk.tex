...
\begin{frame}
  ...
  \note<1>{\todo{say something to pic}}
\end{frame}
...
\begin{frame}{Vorarbeit}
  \note<1>{
    ...
    \begin{itemize}
      \item Hermes (2003) Universität Lancaster
      \item NetBoards (2014) Universität Cambridge
    \end{itemize}
  ...
  Projekte mit ähnlichem Ansatz:
  \begin{itemize}
    \item Hermes Projekt (2003)
    \item NetBoards Projekt (2014)
  \end{itemize}
  \todo{hier Bilder von beiden Projekten}
  % \pause
...
\begin{frame}{Bauhausboards}
  Bauhausboards als Webapplikation\\
  \note<1>{
    Entscheidung fiel auf Web-Applikation\\(andere Möglichkeit wäre Android-/iOS-/Windows-App gewesen - Stichwort Portierung)
  }
  \pause
  \only<2> {
    \todo{hier eine Grafik für die Server-Client Architektur}
  }
  \note<2>{
    \begin{itemize}
      \item Server
      \begin{itemize}
        \item NodeJS als Serversprache auf Basis von Express.js (Framework zum Erstellen einer einfachen Webseite/Webanwendung)
        \item SQLite als Datenbanksystem
      \end{itemize}
      \item Client
      \begin{itemize}
        \item HTML+CSS Webseite (grobes Gerüst)
        \item Javascript mit JQuery für HTML-DOM Manipulation
        \item Paper.js als Framework für die Editorkomponente
      \end{itemize}
    \end{itemize}
  }
  % - Web-Applikation
...
\end{frame}
\begin{frame}{Bauhausboards - Kommunikation}
  \todo{Grafik für die Interaktion Client-Server (Sequenzdiagramm oder so)}
  \note<1>{
    Kommunikation zwischen Client und Server:
    \begin{itemize}
      \item erster Aufruf: Auslieferung der Webseite (Grundgerüst mit CSS und allen Javascript Funktionen)
      \item alle dynamischen Daten: Nachladen auf Client-Seite per ajax (Asynchronous Javascript and XML)
      \item CRUD-Prinzip kommt zum Einsatz
    \end{itemize}
  }
...
\begin{frame}{CRUD-Prinzip}
  \todo{überflüssig? - vllt so einfach erklären}\\
  \textbf{C}reate\\
  \textbf{R}ead\\
  \textbf{U}pdate\\
  \textbf{D}elete
  \note<1>{
    \begin{itemize}
      \item basiert auf Standard HTTP-Anfragen
      \item C(create) R(ead) U(pdate) D(elete)
      \item Server hat Interface für diese Anfragen
      \item wartet auf Anfragen + liefert anfragenspezifisch Daten aus / ändert Serverzustand
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Komponenten}
  \note<1>{
    \begin{itemize}
      \item Editor - Da Simulation von Tafel/Whiteboard
      \begin{itemize}
        \item Editorfunktionen
          \begin{itemize}
            \item Zeichnen mit verschiedenfarbigen Stiften
            \item Schwammfunktion zum Entfernen von gezeichneten/geschriebenen Elementen
            \item Anheften von Bildern/Zetteln per bspw.: Magneten
