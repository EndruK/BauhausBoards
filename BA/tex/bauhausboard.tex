...
\\
\todotext{Fußnote für lib-quotations}\\
\todotext{Literaturverzeichnis ein wenig zu kurz -> vllt noch deren related work noch ein wenig analysieren}\\
\todotext{Quellcode vom System dann auf CD im Endeffekt}
\todotext{DIE sidebar!! prüfen!}
...
\\
Die Applikation sollte nicht bei jeder kleinsten Interaktion mit der Webseite diese neu geladen werden müssen. Das hätte einen viel zu großen Overhead\footnote{\todo} erzeugt, da bei jeder Kommunikation statische Daten übertragen worden wären.
...
\\
Um auf der Webseite navigieren zu können musste es dafür ein Menü geben, das auf den Tablets nicht viel Platz benötigte. Die beste Variante dafür war eine Sidebar\footnote{\todo}, die standardmäßig eingeklappt war und sich per Klick oder Swipe-Geste\footnote{\todo} öffnen ließ.
...
In diesem Header\footnote{\todo} konnte man das Profilbild, den Namen und eine Beschreibung des aktuell ausgewählten Nutzers darstellen.
...
\begin{figure}[h!]
  ...
  \caption{Bauhausboards - Hauptseite\todotext{andres Bild mit mehr Zeichnung}}
  \label{img:finalHauptseite}
\end{figure}

\subsection{Funktionen}\label{Funktionen}
\todotext{vllt ``Clientfunktionen'' als Kapitelname von ``Funktionen''}\\
...
Am wichtigsten war, dass sich die Seite nicht neu lud, wenn man durch sie navigierte. Dies wurde mit jQuery\cite{jquery:website} realisiert. jQuery ist eine Bibliothek zur einfachen Manipulation der HTML-Struktur einer Webseite. Mit ihr war es einfach, die dargestellte Ansicht anzupassen.
\\
...
Damit sie auf Tablets leichter zu öffnen war, nutzte ich TouchSwipe\cite{touchswipe:website}. Mit dieser Bibliothek konnten Swipe-Gesten zum Aufruf von Funktionen genutzt werden.
\begin{figure}[h!]
  \centering
...
\begin{figure}
  \centering
    \frame{\includegraphics[width=0.85\textwidth]{./img/EditorObjectSelection.png}}
